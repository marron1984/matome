...
記事は既定で**リーダーモード**で開きます。サーバ側で記事ページから本文だけを抽出し、
広告・アフィリエイトを取り除いた画面をアプリ内に表示します。

- 除去されるもの: `script` / `iframe` / 広告ブロック（ad系のclass・id）/ Amazon・楽天・A8等のアフィリエイトリンク（リンクだけ外して文章は残す）
- 残るもの: レス本文・引用・画像・普通のリンク
- 出力はホワイトリスト方式で組み立て直すため、元ページのスクリプトがアプリ内で動くことはありません
- 本文が取り出せないページは「元のページを開く」への誘導にフォールバックします
- 画面上部の「元記事」からいつでも本家ページへ行けます（設定でリーダーモード自体をオフにもできます）
- 抽出対象はストアに存在する記事URLのみ（`/api/extract` が任意URLの取得口にならないための制限）

リーダーモードがオフの場合、記事は**同じタブで開きます**。ブラウザの「戻る」で一覧に戻ると、開いていたタブ・検索語・読み込み済みの件数・スクロール位置まで元に戻ります
...
src/extract.js        リーダーモード用の本文抽出・広告除去
...
| `GET /api/extract?url=` | リーダーモード用の本文抽出（購読中の記事URLのみ受け付け） |
